...
pub fn help_text(launcher: Launcher) -> String {
    const ACCOUNT_HELP: &str = "\nAccount diagnostics and rotation:\n  neomax doctor [--json]                    inspect local evidence without changing state\n  neomax PROVIDER ACCOUNT rotate            select an eligible same-provider replacement\n    --with ACCOUNT [--dry-run] [--json]     choose a number, profile name, or saved email\n  neomax config set reset-aware-ranking true|false\n                                           opt into finer weekly-reset ranking\nRotation swaps OAuth credentials for Claude/Codex; session files stay put.\nAn open provider may need authentication reload or resume.\n";
    const TUI_HELP: &str = "\nTerminal workspace:\n  neomax tui                               fleet, tasks, usage, and interactive launch\n\nCodex speed:\n  --codex-standard                         standard mode (default)\n  --codex-fast                             opt into fast mode for Codex launches\n  NEOMAX_CODEX_FAST=1                       enable for this process and its Codex workers\n\nThe TUI Launch page offers the same speed choice. Inside an open Codex\nterminal, /fast on, /fast off, and /fast status control the native setting.\n";
    ...
    match launcher {
        Launcher::Universal | Launcher::ProviderOrchestrator(_) => {
            format!(
                "{text}{}{TUI_HELP}{ACCOUNT_HELP}",
                SOLO_HELP.replace("{name}", name)
            )
        }
