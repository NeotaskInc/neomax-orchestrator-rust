...
function modelQuotaRows(account) {
  ...
  return Object.entries(windows).map(([family, window]) => {
    ...
    const value = expired ? null : window.used_percent;
    const width = Math.min(100, Math.max(0, num(value)));
    const reset = window.resets_at ? (expired ? 'reset passed; awaiting refresh' : 'resets ' + fmtTime(window.resets_at)) : 'reset unknown';
    return `<div class="bar-row"><span class="bar-label">${esc(label)}</span><span class="bar"><i class="${width >= 99 ? 'warn' : ''}" style="width:${width}%"></i></span><span>${Number.isFinite(value) ? percent(value) : 'unknown'}</span></div><div class="small-text quota-detail">${esc(reset)}${width >= 99 ? ' · this model family is unavailable' : ''}</div>`;
  }).join('');
}
...
export function renderUsage(report) {
  ...
  const warnings = (report?.warnings || []).map(warning => `<p class="hint" role="status">${esc(warning)}</p>`).join('');
  ...
  const pricing = Object.entries(report?.pricing || {}).map(([model, price]) => `<tr><td class="mono">${esc(model)}</td><td>${fmtUsd(price.in)}</td><td>${fmtUsd(price.out)}</td><td>${fmtUsd(price.cr)}</td><td>${fmtUsd(price.cw)}</td><td>${price.cw1h === null || price.cw1h === undefined ? '-' : fmtUsd(price.cw1h)}</td></tr>`).join('');
  document.querySelector('#usage-breakdowns').innerHTML = `${warnings}${breakdowns}<details class="usage-block"><summary>Pricing catalog <span class="hint">${fmtCount(Object.keys(report?.pricing || {}).length)} models</span></summary><div class="table-scroll"><table id="usage-pricing"><thead><tr><th>model</th><th>input / M</th><th>output / M</th><th>cache read / M</th><th>cache write / M (Claude 5m)</th><th>Claude 1h write / M</th></tr></thead><tbody>${pricing || '<tr><td class="empty" colspan="6">No pricing records.</td></tr>'}</tbody></table></div></details>${renderLocalDetails(report)}`;
}
